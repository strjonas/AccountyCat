...
struct MonitoringStatsSnapshotTests {
    ...
    func aggregatesDecisionMixSkipCausesTokensAndProfiles() async throws {
        ...
        #expect(snapshot.decisionMix.first(where: { $0.label == "tolerated" })?.value == "0 · 0%")
        ...
        #expect(snapshot.profileBreakdown.first(where: { $0.label == "Coding" })?.value == "1x · F 1 / T 0 / D 0 / U 0")
        #expect(snapshot.profileBreakdown.first(where: { $0.label == "General" })?.value == "1x · F 0 / T 0 / D 0 / U 1")
